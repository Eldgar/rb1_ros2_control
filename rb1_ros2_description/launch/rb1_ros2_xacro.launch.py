...
def generate_launch_description():
   ...
   # Include the Gazebo launch file with the modified launch arguments
    ...
    # Apply joint effort after a delay, prevents elevator from drifting after a set position
    apply_effort = TimerAction(
        period=5.0,  # Wait for 5 seconds after the launch starts
        actions=[
            ExecuteProcess(
                cmd=[
                    'ros2', 'service', 'call',
                    '/apply_joint_effort', 'gazebo_msgs/srv/ApplyJointEffort',
                    "{joint_name: 'robot_elevator_platform_joint', effort: 0.2, start_time: {sec: 0, nanosec: 0}, duration: {sec: -1, nanosec: 0}}"
                ],
                output='screen'
            )
        ]
